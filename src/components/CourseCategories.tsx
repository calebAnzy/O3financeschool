...
import { ArrowRight, CreationIcon, ManagementIcon, SustainabilityIcon } from "./Icons"
...
const CourseCategories = () => {
  ...
  return (
    <div>
      ...
      <section className="flex flex-col gap-3 cursor-pointer mt-4">
        {categories.map((category) => (
          <div
            key={category.name}
            className="
              flex items-center justify-between gap-3 
              border rounded-xl border-[#E4E7EC] p-2.5
              hover:shadow-sm transition-shadow duration-200
              sm:flex-row flex-col sm:items-center
            "
          >
            {/* Left side */}
            <div className="flex items-center sm:items-center gap-2 w-full sm:w-auto">
              <IconContainer color={category.bgcolor} icon={category.icon} />

              <div className="flex flex-col">
                <h4 className="text-base sm:text-lg leading-none font-medium">
                  {category.name}
                </h4>
                <section className="flex flex-wrap gap-2 items-center mt-1 [&_div]:flex [&_div]:items-center [&_div]:gap-1">
                  <div>
                    {/* Faculties */}
                    <svg
